...
class TestDatasets(unittest.TestCase):
    ...
    def prepare_builder(self, build_from) -> db.BaseDatasetBuilder:
        ...
        optical_flow_checkpoint = weights_dir_path/weights_file

        depth_estimator_name = 'struct2depth'
        weights_file = 'model-199160'
        self.assertTrue(list(weights_dir_path.glob(weights_file + '*')))
        depth_checkpoint = weights_dir_path/weights_file
        ...
        builder = dataset_builder(sequence_directory,
                                  ...
                                  optical_flow_checkpoint=optical_flow_checkpoint.as_posix(),
                                  estimate_depth=estimate_depth,
                                  depth_estimator_name=depth_estimator_name,
                                  depth_checkpoint=depth_checkpoint.as_posix(),
                                  memory_safe=True,
                                  **computation_kwargs)
    ...
    def test_tum(self) -> None:
        ...
        builder = self.prepare_builder(db.TUM)
        builder.build()
    ...
    def test_discoman(self) -> None:
        ...
        builder = self.prepare_builder(db.DISCOMAN)
        builder.build()
        df = builder.dataframe
    ...
    def test_kitti(self) -> None:
        ...
        builder = self.prepare_builder(db.KITTI)
        builder.build()
        df = builder.dataframe
    ...
    #     builder = self.prepare_builder(db.VIDEO)
    #     builder.build()
    #     df = builder.dataframe
    #
    #     for path in df["path_to_rgb"]:
