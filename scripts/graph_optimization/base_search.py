...
class BaseSearch:
    ...
    """
    def __init__(self, vis_dir, pred_dir):
    ...
    def get_strides_sigmas(self, vals, current_level, max_depth):
        if current_level == max_depth:
            ...
            coefs = self.get_strides_sigmas(vals, current_level + 1, max_depth)
            new_coefs = list()
            for v in vals:
                for c in coefs:
    ...
    def get_rpe_mode(config):
        random_stride = list(config.keys())[0]
