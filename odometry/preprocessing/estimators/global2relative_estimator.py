...
class Global2RelativeEstimator(BaseEstimator):
    ...
    def run(self, row, dataset_root=None):
        dof = row[self.input_col]
        next_input_cols = [col + '_next' for col in self.input_col]
        next_dof = row[next_input_cols]
        ...
        row[self.output_col] = np.concatenate([relative_euler_angles, relative_translation])
        return row
