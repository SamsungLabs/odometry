...
class KITTIDataset(OpticalFlowDataset):
    ...
    def __init__(self, mode='train_with_val', ds_root=_KITTI_ROOT, options=_DEFAULT_DS_TRAIN_OPTIONS):
        ...
        assert(self.opts['type'] in ['noc', 'occ', 'npy'])
    ...
    def _build_ID_sets(self):
        ...
        while idx < len(frames) - 1:
            self._IDs.append((frames[idx], frames[idx + 1],
                              '{}_{}.npy'.format(frames[idx][:len(frames[idx])-4], frames[idx+1][:len(frames[idx])-4])))
            idx += 2
        ...
        while idx < len(frames) - 1:
            flow_ID = '{}_{}.npy'.format(frames[idx][:len(frames[idx])-4], frames[idx+1][:len(frames[idx])-4])
